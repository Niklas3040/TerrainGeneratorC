...
#include "MarchinCubesChunk.h"

#include "Field/FieldSystemNoiseAlgo.h"


// Sets default values
AMarchinCubesChunk::AMarchinCubesChunk()
{
	...
	RealtimeMeshComponent->AttachToComponent(GetRootComponent(), FAttachmentTransformRules::KeepRelativeTransform);
...
void AMarchinCubesChunk::BeginPlay()
{
	Super::BeginPlay();
...
// Called every frame
void AMarchinCubesChunk::Tick(float DeltaTime)
{
	Super::Tick(DeltaTime);

}

void AMarchinCubesChunk::SetupNoise() const
{
	Noise->SetupFastNoise(NoiseType, Seed, Frequency);
}

void AMarchinCubesChunk::Setup()
{
	// Initialize Voxels
...
void AMarchinCubesChunk::Generate2DHeightMap(const FVector Position)
{
...
void AMarchinCubesChunk::Generate3DHeightMap(const FVector Position)
{	
	for (int x = 0; x <= Size; ++x)
	{
		for (int y = 0; y <= Size; ++y)
		{
			...
			{
				Voxels[GetVoxelIndex(x,y,z)] = Noise->GetNoise3D(x + Position.X, y + Position.Y, z + Position.Z);	
			}
		}
	}
}

void AMarchinCubesChunk::GenerateMesh()
{
	// Triangulation order
...
void AMarchinCubesChunk::March(const int X, const int Y, const int Z, const float Cube[8])
{
	int VertexMask = 0;
	...
	for (int i = 0; i < 5; ++i)
	{
		...
		MeshData.Positions.Append({V1, V2, V3});
...
int AMarchinCubesChunk::GetVoxelIndex(const int X, const int Y, const int Z) const
{
	return Z * (Size + 1) * (Size + 1) + Y * (Size + 1) + X;
}

float AMarchinCubesChunk::GetInterpolationOffset(const float V1, const float V2) const
{
